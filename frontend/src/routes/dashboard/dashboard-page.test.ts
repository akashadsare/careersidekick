...
describe('DashboardPage', () => {
  ...
  it('clears all incident filters from header action', async () => {
    localStorage.setItem(
      'careersidekick_dashboard_prefs',
      JSON.stringify({
        days: '14',
    ...
    const incidentCalls = fetchMock.mock.calls
      .map((call) => String(call[0]))
      .filter((url) => url.includes('/api/v1/executions/incidents?'));
    expect(incidentCalls.some((url) => url.includes('days=14') && url.includes('state=critical'))).toBe(true);
    expect(incidentCalls.some((url) => url.includes('days=30') && !url.includes('state='))).toBe(true);
  });
